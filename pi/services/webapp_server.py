...
class WebappServer:
    def __init__(
        ...
        on_browser_audio: Callable | None = None,
        on_demo_command: Callable | None = None,
    ) -> None:
        ...
        self._on_browser_audio = on_browser_audio
        self._on_demo_command = on_demo_command
    ...
    def _register_routes(self) -> None:
        ...
        async def demo() -> FileResponse:
            return FileResponse(str(WEBAPP_DIR / "demo.html"))
            ...
            try:
                while True:
                    ...
                    if "bytes" in msg and msg["bytes"]:
                        if self._on_browser_audio:
                            asyncio.create_task(self._on_browser_audio(msg["bytes"]))
                    elif "text" in msg and msg["text"]:
                        try:
                            import json as _json
                            parsed = _json.loads(msg["text"])
                            if isinstance(parsed, dict) and str(parsed.get("t", "")).startswith("demo_"):
                                if self._on_demo_command:
                                    asyncio.create_task(self._on_demo_command(parsed))
                        except Exception:
                            pass
